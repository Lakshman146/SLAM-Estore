...
public class CustomerController {
	...
	orderModel om;
	@Autowired
	public CustomerController(cartDAO cartdao, customerDAO customerdao,StockUpdaterDAO stckdao, orderModel om,BLLClass2 bl2, BLL bl1, ProductDAO productdao,
	...
	@ResponseBody
	public String handleFormSubmission(@RequestParam(value = "name") String name,@RequestParam(value = "custSAddress")String caddress,@RequestParam(value = "custSpincode")String pincode,HttpSession session) {
		custCredModel cust = (custCredModel) session.getAttribute("customer");
	...
	public String invoice(@RequestParam("paymentReference") String id, @RequestParam("total") String total,
			Model model, HttpSession session,@Validated orderModel om) {
		custCredModel cust1 = (custCredModel) session.getAttribute("customer");
		wallet Wallet = wdao.getWalletAmount(cust1.getCustId());
